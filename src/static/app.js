    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHTML = details.participants && details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(email => `<li class="participant-item"><span class="participant-badge">${escapeHtml(email)}</span><button type="button" class="participant-remove" data-email="${escapeHtml(email)}" aria-label="Remove ${escapeHtml(email)}">×</button></li>`).join("")}</ul>`
